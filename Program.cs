    ...
    class Program
    {
        ...
        public static void Intersection(int[] nums1, int[] nums2)
        {
            try
            {
                ...
                {
                    int num_comp = nums1[i]; //Copy the value of each number of nums1 in num_comp
                    if(nums2.Contains(num_comp) && !result.Contains(num_comp))
                    {
                        result.Add(num_comp);
                    }
        ...
        private static int LuckyNumber(int[] nums)
        {
            try
            {
                ...
                for (int i = nums.Length - 2; i >= 0; i--)
                {
                    ...
                    if (nums[i] == counter && (i == 0 || nums[i - 1] != nums[i])) return nums[i];
                }
        ...
        private static int MinCostToClimb(int[] costs)
        {
            try
            {
                ...
                {
                    int current_step = costs[i] + Math.Min(step1, step2);
                    step1 = step2;
                    step2 = current_step;
                }
                //return 0;
